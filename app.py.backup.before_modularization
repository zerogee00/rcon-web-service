...
@app.route('/slack/mc', methods=['POST'])
@verify_slack_signature
def slack_mc():
    """Handle Slack slash command for Minecraft"""
    ...
    if not text:
        ...
        return jsonify({
            ...
            'text': f'Usage:\n• `/mc <server_id> <command>` - Execute command on specific server\n• `/mc <command>` - Execute on your default server\n\nAvailable servers:\n' + '\n'.join(server_list) + default_text + '\n\nContext Commands:\n• `/mc context set <server_id>` - Set default server\n• `/mc context get` - Show current default\n• `/mc context clear` - Clear default\n• `/mc help` - Show command help'
        })
    ...
    if parts[0] in SERVERS:
        # Traditional format: /mc <server_id> <command>
        if len(parts) < 2:
            return jsonify({
                'response_type': 'ephemeral',
                'text': 'Invalid format. Usage: /mc <server_id> <command>'
            })
        server_id = parts[0]
        command = parts[1]
    else:
        # New format: /mc <command> (uses default server)
        ...
        if not server_id:
            return jsonify({
                'response_type': 'ephemeral',
                'text': '❌ No default server set. Use `/mc context set <server_id>` to set one, or use `/mc <server_id> <command>`'
            })
...
@app.route('/slack/backup', methods=['POST'])
@verify_slack_signature
def slack_backup():
    """Handle backup requests from Slack"""
    user_name = request.form.get('user_name', 'unknown')
    text = request.form.get('text', '').strip()
    ...
    logger.info(f"Admin user {user_name} requesting backup with args: {text}")
...
**Examples:**
• `/backup incremental 7eaa7ab6` - Incremental backup for specific server
• `/backup full` - Interactive full backup (all servers)
• `/backup incremental` - Incremental backup for all servers"""
...
def handle_backup_status():
    """Show backup status and recent logs"""
    try:
        log_file = '/var/log/minecraft-backup.log'
        if os.path.exists(log_file):
            # Get last 10 lines of backup log
